 ...
 */
public class SimplAirbrakeNotifier {
  ...
  private static class AirbrakeExceptionHandler implements UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread t, Throwable e) {
      if (shouldNotify(e)) SimplAirbrakeNotifier.notify(e);
      defaultExceptionHandler.uncaughtException(t, e);
    }
  }

  private static boolean shouldNotify(final Throwable throwable) {
    if (throwable == null) return false;
    if (throwable.getMessage() != null && throwable.getMessage().contains("simpl")) return true;
    for (StackTraceElement stackTraceElement : throwable.getStackTrace()) {
      if (stackTraceElement == null) continue;
      if (stackTraceElement.toString().contains("simpl")) return true;
    }
    return false;
  }
  ...
  public static void register(Context context, String apiKey, String environmentName,
    ...
    if (apiKey != null) {
      SimplAirbrakeNotifier.apiKey = apiKey;
    } else {
    ...
    if (environmentName != null) {
      SimplAirbrakeNotifier.environmentName = environmentName;
    }

    // Check which exception types to notify
    SimplAirbrakeNotifier.notifyOnlyProduction = notifyOnlyProduction;

    // Connect our default exception handler
    UncaughtExceptionHandler currentHandler = Thread.getDefaultUncaughtExceptionHandler();
    if (!(currentHandler instanceof AirbrakeExceptionHandler) && (environmentName.equals(
        ENVIRONMENT_PRODUCTION) || !notifyOnlyProduction)) {
  ...
  public static void updateEnvironmentName(final String newEnvironmentName) {
    SimplAirbrakeNotifier.environmentName = newEnvironmentName;
  }
  ...
  public static void setExtraData(Map<String, String> extraData) {
    SimplAirbrakeNotifier.extraData = extraData;
  }
  ...
  private static void sendExceptionData(File file) {
    try {
      ...
      try {
        ...
        Log.d(LOG_TAG, "Sent exception file "
            + file.getName()
            + " to airbrake. Got response code "
